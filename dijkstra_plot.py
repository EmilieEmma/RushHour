...
strFileName="puzzles/intermediaire/jam17.txt"
...
[totalFiles.append(expertDirectory + f) for f in listdir(expertDirectory) if isfile(join(expertDirectory, f))]
[totalFiles.append(debutantDirectory+ f) for f in listdir(debutantDirectory) if isfile(join(debutantDirectory, f))]
[totalFiles.append(avanceDirectory + f) for f in listdir(avanceDirectory) if isfile(join(avanceDirectory, f))]
[totalFiles.append(intermediaireDirectory + f) for f in listdir(intermediaireDirectory) if isfile(join(intermediaireDirectory, f))]
print totalFiles
        ...
        for caseValue in cases:
            if caseValue.find('g') == 0:        # on teste si la voiture rouge est sur la bonne ligne
                ...
                """
                    ...
                    if len(caseValue) ==2 :
                    caseValue = caseValue[0] + "0" + caseValue[1] 
                    print caseValue
                """
            if caseValue != '0' :    #presence d une voiture c: voiture , t: camion
                #print "CREATION CASEVALUE " , caseValue
                if len(caseValue) == 2 :
                    caseValue = caseValue[0] + "0" + caseValue[1]
                if not dicoVoitures.has_key(caseValue) :
                    #print " caseValue ", caseValue
    ...
    for y in xrange(dimensionsGrille[0]) :
        for x in xrange(dimensionsGrille[1]):
            ...
            if configuration2D[y][x] != "0" and configuration2D[y][x] != configuration2D[y][x-1] and configuration2D[y][x] != configuration2D[y-1][x]:
                ...
                if len(conf) ==2 :
                    conf = conf[0] + "0" + conf[1]
                if i < 10 :
                    #print conf," AVANT : ", clef + "0"+str(i)+ conf
...
def majDicoCoord(clef):
    ...
    #print "SETTTTT setCoupleVoitPos : ", setCoupleVoitPos
...
"""
fonction qui sequence les informations de la clef tous les 5 caracteres
pour retrouver idVoiture avec PositionMarqueur associe 
exemple de clef : 10t0207c0114g0125c0232t03 =>10t02 07c01 14g01 25c02 32t03
vehicule t02 en position 10
...
def trouverCouplesVoituresPositionInitiale(clef):
    ...
    Iter = len(clef)/5      #definit le nombre d 'iterations pour la boucle for
    first= 0
    dataSet = Set()
    for i in range(Iter):       # iteration de 4 en 4 pour isoler les positions de chaque voiture ( "17g1") a partir de la clef
        #print clef        
        #print "trouverCouplesVoituresPositionInitiale : ",clef[first+2:5+first],clef[first:5+first-3]        
        dataSet.add((clef[first+2:5+first],clef[first:5+first-3]))
        first = first+ 5    
    ...
    step= 4
    dataSet = Set()
    #for index in range(0,len(clef),step):       # iteration de 4 en 4 pour isoler les positions de chaque voiture ( "17g1") a partir de la clef
    while index < len(clef)  :  
    #if first+2:4+first
        #print "first+2 ", first+2," 4+first ",4+first
        #print " TESTTT ", clef[first+3:first+7], "==>", (clef[first+3:first+7]).isdigit() , " === " , clef
        if not (clef[first+3:first+7]).isdigit() :
            #print "clef 1 ", clef[first+2:4+first], " clef 2 : ", clef[first:4+first-2]
            ...
            #print "clef 1 ", clef[first+2:5+first], " clef 2 : ", clef[first:4+first-2]
            dataSet.add((clef[first+2:5+first],clef[first:4+first-2]))
            #print first ,"=", index            
            first = first+ 5    
            index = index + 5
            step=5
...
def trouverCouplesVoituresPositionVoisine(clef):
    ...
    first= 0
    dataSet = Set()
    for i in range(Iter):       # iteration de 5 en 5 pour isoler les positions de chaque voiture ( "17g01") a partir de la clef
        #print clef        
        #print "trouverCouplesVoituresPositionInitiale : ",clef[first+2:5+first],clef[first:5+first-3]        
        dataSet.add((clef[first+2:5+first],clef[first:5+first-3]))
        first = first+ 5    
...
def setPositionsOccupees(setCouplesVoituresPositions) : #setCouplesVoituresPositions => set de couple (voiture, position)
    ...
    for v,p in setCouplesVoituresPositions:
        #if v == "g01":
        #    v="g"
        #print "setCouplesVoituresPositions ", setCouplesVoituresPositions
        #print "dicoVoitures ",dicoVoitures
...
# test si orientation verticale ou horizontale
    ...
    #print "SET COORD VOISINS %%%%%%%%%%%%%%%%%%%%%%% ", setCoordVoisins
...
def trouverVoisinsValides(clef, dPositionsOccupees):
    ...
    for voit,position in setVoituresPos :
        ...
        #print "%%%%%%%%%%%%%%%% LISTE CASESVALIDES ", newMove
        for i in newMove:
            if i <10:
        ...
        #print " *********************** liste Positions ", listPositions
...
def creerConfigVoisines(clef,voisinsValides):
    for key,value in voisinsValides.items() :
        for ind in range(0,len(value)):
            ...
            oldSubStr = clef[indexSubStr-2:indexSubStr+3]       
            #print "#################### oldSubStr ", oldSubStr            
            newSubStr = value[ind]
    ...
    #print "dicoConfigVoisines ", dicoConfigVoisines
...
for i in totalFiles:    
    ...
    coutRHM = -1
    ###  APPELS DES FONCTIONS
    configuration2D,dimensionsGrille = lectureFichier(i)    
    ...
    edges, clefsBut = genereEdges(dicoConfigVoisines, "17g01") 
    #print "EDGES : ", edges , "\n\n"
    ...
    print "fichier : ",i, "temps d execution ", (fin - debut), " secondes "
    plotFileName.append(i)
    plotTmps.append((fin - debut))
    plotNbConfig.append(len(clefsBut))


print "generation du plot ",plotTmps,plotNbConfig